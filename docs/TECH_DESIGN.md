...
| 全局热键 ⌥C（S3） | computer_use 合成按键 | ⚠️ 无法自动化验证：合成按键事件不触发 Carbon 全局热键。**需真机手动按键验证** |

**验收结论**：代码层 + UI 层（AX）验证全部通过；剩余 2 项（拖拽移动、⌥C 热键）因合成事件限制需用户真机手动验证。
