...
## 7. 真机验收记录（2026-08-11，computer_use 驱动）

| 验收项 | 方法 | 结果 |
|--------|------|------|
| 面板显示 | `-showPanelOnLaunch` 调试钩子启动（绕过全局热键） | ✅ CGWindowList 确认 520×160 面板在屏 |
| 条目数显示（增强 3.4） | AX 树读取 | ✅ "0 个文件" → 挂载后 "1 个文件" |
| 来源路径显示（需求 2） | AX 树读取 | ✅ 条目显示 `/Users/leoli/Downloads` |
| 清空功能（缺陷 2） | AX 点击垃圾桶按钮 | ✅ "1 个文件"→"0 个文件"，条目消失；源文件完好（数据零丢失） |
| 失效检测 + 清理（增强 3.2） | 写入失效条目 → AX 点击扫把 | ✅ "2 个文件"→"1 个文件"，失效条目清理、存活条目保留 |
| 拖出时面板不动（缺陷 1） | CGEvent 模拟拖拽，对比前后位置 | ✅ 面板位置保持 (1026,105) 不变 |
| 拖出真实移动（S2） | CGEvent 模拟拖拽 | ⚠️ 无法自动化验证：终端无辅助功能权限，合成鼠标事件被系统丢弃。**需真机手动拖拽验证** |
| 全局热键 ⌥X（S3） | computer_use 合成按键 | ⚠️ 无法自动化验证：合成按键事件不触发 Carbon 全局热键。**需真机手动按键验证** |

**验收结论**：代码层 + UI 层（AX）验证全部通过；剩余 2 项（拖拽移动、⌥X 热键）因合成事件限制需用户真机手动验证。

### 7.1 手动验收补录（2026-08-11，用户真机操作）

| 验收项 | 结果 | 说明 |
|--------|------|------|
| 拖出真实移动（S2） | ✅ 用户确认通过 | 拖到桌面文件真实移动、源位置消失、货架条目移除 |
| 全局热键 ⌥X（S3） | ✅ 用户确认通过 | 面板唤起/隐藏正常 |

**验收状态：全部通过，Spike + 技术设计细化阶段闭环。** 进入 MVP 开发（12 周目标）。
